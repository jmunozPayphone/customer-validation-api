...
namespace CustomerValidation.SharedKernel.Guards;
...
public static class Guard
{
    ...
    public static void ThrowIfPositive([NotNull] decimal value, [CallerArgumentExpression(nameof(value))] string? paramName = null)
    {
        if (value > 0)
        {
            throw new ArgumentOutOfRangeException(paramName, value, $"{paramName} ('{value}') must be a non-positive value.");
        }
    }
